...
xx = sin(betas) .* cos(alphas); %
yy = sin(betas) .* sin(alphas); %
zz = cos(betas);                %
...
S.alphas = alphas; %phi
S.betas = betas; % Theta
S.hh = squeeze(table(:, :, 1)).'; % E_theta_real E_theta_imag
S.vv = squeeze(table(:, :, 2)).'; % E_phi_real E_phi_imag
S.hv = squeeze(table(:, :, 3)).'; % (E_theta_real E_theta_imag)*(E_phi_real E_phi_imag)
